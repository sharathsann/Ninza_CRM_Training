...
public class SalesOrderTest extends BaseClass{
	GenericPage gp;
	 ExcelUtility eUtil;
@Test(priority =8,dependsOnGroups = "PurchaseOrder",groups = "SalesOrder")
	public void SalesOrderFillDetails() throws InterruptedException, EncryptedDocumentException, IOException
	{
		 gp=new GenericPage(driver);
		 eUtil=new ExcelUtility();
		
		gp.headerModules_selection_campains_to_InvoiceModule("Sales Order");
		...
		gp.popup_page_DropdownSelection_searchCampaign_Select(3,"Contact Name", "ContactsPage", 4, 1);
		gp.popup_page_DropdownSelection_searchCampaign_Select(4,"Product Name", "ProductsPage", 0, 1);
	...
	@Test(priority=13,dependsOnGroups = "InvoiceOrderDelete",groups = "SalesOrderDelete")
public void SalesOrderDelete() throws EncryptedDocumentException, InterruptedException, IOException
{
	     gp=new GenericPage(driver);
		 eUtil=new ExcelUtility();
		gp.headerModules_selection_campains_to_InvoiceModule("Sales Order");
